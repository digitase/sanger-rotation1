...
# contained_snps="$out_dir/contained.snps.$dataset.$assoc.signif.txt"
contained_snps="$out_dir/contained.snps.$dataset.$assoc.all.txt"

# Output unique snp ids within each chr
mkdir -p "$out_dir/snp_ids/$dataset/$assoc"
for ((chr = 1; chr <= 22; chr++)) {
    echo Getting ids of contained snps from chr $chr...
    awk -F'\t' -v chr="$chr" '($1 == chr) {print $24}' "$contained_snps" | sed 's/"//g' | sort -u > "$out_dir/snp_ids/$dataset/$assoc/$chr.txt"
}
...
mkdir -p "$out_dir/gen/$dataset/$assoc/logs"
...
for ((chr = 1; chr <= 22; chr++)) {
    bsub \
        ...
        -o "$out_dir/gen/$dataset/$assoc/logs/$chr.gen.bsub_o.log" \
        -e "$out_dir/gen/$dataset/$assoc/logs/$chr.gen.bsub_e.log" \
        "zcat $in_gen_files_dir/$chr.gen.gz | awk '{if (NR == FNR) {id[\$1] = 1} else {if (id[\$2]) {print}}}' $out_dir/snp_ids/$dataset/$assoc/$chr.txt - | \
            split -a 10 -d -l $chunk_size - $out_dir/gen/$dataset/$assoc/$chr.gen."
}
